...
export type DDay = {
    ...
    endDate?: Date; // end date for multi-day events - used by layout system and date range calculations
    description: string; // detailed description of the event - shown in DDayIndicator details dialog
    days: string; // calculated D-day count (e.g., "D-5", "Today", "D+3") - computed by useDDays hook
...
// defines the visual position of an event in a multi-day layout - used by DDayIndicator and calendar grid
...
export interface CalendarGridProps {
    ...
    activeDDay: DDay | null; // currently dragged event (for drag & drop) - from calendar page drag state
}
...
export interface AddDDayDialogProps {
    isOpen?: boolean; // whether the dialog is open (for controlled mode) - from CalendarGrid state
    onOpenChange?: (open: boolean) => void; // callback when dialog open state changes - passed to CalendarGrid
    initialDate?: Date | null; // pre-selected date for the new event - from CalendarGrid day selection
...
export interface DDayIndicatorProps {
    ...
    length?: "short" | "long"; // display length (truncated vs full title) - used by ShowAllEvents for table display
    position?: EventPosition; // visual position in multi-day layout - calculated by getEventPosition function
    dayIndex?: number; // index of the day in the grid - used by CalendarGrid for week highlighting
    droppableId?: string; // id for drag & drop functionality - used by CalendarGrid drag system
}
...
export interface DDayFormData {
    ...
    endDate?: Date; // end date for multi-day events - used by layout system
...
export interface DDayFormProps {
    initialData?: Partial<DDayFormData>; // pre-populated form data - from existing event or user input
...
// props for the "show all events" dialog component - used by CalendarGrid when there are 4+ events on a day
